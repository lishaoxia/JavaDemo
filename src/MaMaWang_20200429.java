...
public class MaMaWang_20200429 {
    ...
    public static List<String> getFileContext(String path) throws Exception {
        ...
        while (line!= null) {
            ...
            if(ob.getBooleanValue("bool_isComment")==false)
            {
                ...
                String kw_id = kw_url.split("/")[(kw_url.split("/").length)-1];
                ob.put("kw_id",kw_id);
                ob.remove("kw_originalID");
                ob.remove("kw_originalUrl");
                ob.remove("long_floor");
                ob.remove("text_originalContent");
